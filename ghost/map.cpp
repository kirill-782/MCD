...
CMap :: CMap( CConfig *CFG, string nCFGFile )
{
	Load( CFG, nCFGFile );
}
...
void CMap :: Load( CConfig *CFG, string nCFGFile )
{
	...
	if( !m_MapLocalPath.empty( ) )
		m_MapData = UTIL_FileRead( m_MapLocalPath );

	// load the map MPQ

	string MapMPQFileName = m_MapLocalPath;
	HANDLE MapMPQ;
	bool MapMPQReady = false;

#ifdef WIN32
	const wstring MapMPQFileNameW = wstring(MapMPQFileName.begin(), MapMPQFileName.end());
