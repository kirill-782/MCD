...
public:
	GAPI CMap( );
	GAPI CMap( CConfig *CFG, string nCFGFile );
	GAPI ~CMap( );

	GAPI bool GetValid( )						{ return m_Valid; }
	GAPI string GetCFGFile( )					{ return m_CFGFile; }
	GAPI string GetMapPath( )					{ return m_MapPath; }
	GAPI BYTEARRAY GetMapSize( )					{ return m_MapSize; }
	GAPI BYTEARRAY GetMapInfo( )					{ return m_MapInfo; }
	GAPI BYTEARRAY GetMapCRC( )					{ return m_MapCRC; }
	GAPI BYTEARRAY GetMapSHA1( )					{ return m_MapSHA1; }
	GAPI unsigned char GetMapSpeed( )			{ return m_MapSpeed; }
	GAPI unsigned char GetMapVisibility( )		{ return m_MapVisibility; }
	GAPI unsigned char GetMapObservers( )		{ return m_MapObservers; }
	GAPI unsigned char GetMapFlags( )			{ return m_MapFlags; }
	GAPI BYTEARRAY GetMapGameFlags( );
	GAPI uint32_t GetMapGameType( );
	GAPI uint32_t GetMapOptions( )				{ return m_MapOptions; }
	GAPI unsigned char GetMapLayoutStyle( );
	GAPI BYTEARRAY GetMapWidth( )				{ return m_MapWidth; }
	GAPI BYTEARRAY GetMapHeight( )				{ return m_MapHeight; }
	GAPI string GetMapType( )					{ return m_MapType; }
	GAPI string GetMapMatchMakingCategory( )		{ return m_MapMatchMakingCategory; }
	GAPI string GetMapStatsW3MMDCategory( )		{ return m_MapStatsW3MMDCategory; }
	GAPI string GetMapDefaultHCL( )				{ return m_MapDefaultHCL; }
	GAPI uint32_t GetMapDefaultPlayerScore( )	{ return m_MapDefaultPlayerScore; }
	GAPI string GetMapLocalPath( )				{ return m_MapLocalPath; }
	GAPI bool GetMapLoadInGame( )				{ return m_MapLoadInGame; }
	GAPI string *GetMapData( )					{ return &m_MapData; }
	GAPI uint32_t GetMapNumPlayers( )			{ return m_MapNumPlayers; }
	GAPI uint32_t GetMapNumTeams( )				{ return m_MapNumTeams; }
	GAPI vector<CGameSlot> GetSlots( )			{ return m_Slots; }

	GAPI void Load( CConfig *CFG, string nCFGFile );
	GAPI void CheckValid( );
	GAPI uint32_t XORRotateLeft( unsigned char *data, uint32_t length );
};
